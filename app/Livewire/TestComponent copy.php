...
public function setTruckStatus($truckId, $status): void
{
    ...
    $this->loadTrucks();
    $this->planMaintenance($truckId, $status);
}
...
public function loadStatusStats()
{
    ...
    // Диспетчер планирует задание (только для free/completed)
    public function planMaintenance($truckId, $taskType): void
    {
        $truck = Truck::with('truckModel')->find($truckId);
        
        if (!in_array($truck->status, ['free', 'completed'])) {
            session()->flash('error', "Грузовик {$truck->number} занят рейсом!");
            return;
        }
        
        // ✅ ТОЛЬКО СОЗДАЕМ ПЛАН - статус НЕ меняем!
        TruckPlannedTask::create([
            'truck_id' => $truckId,
            'task_type' => $taskType,
        ...
        Cache::put("driver_notification_{$truckId}", 
            "📋 По окончании рейса: {$this->getTaskText($taskType)}", 300);
        session()->flash('success', "✅ {$truck->number}: {$this->getTaskText($taskType)}");
        $this->loadTrucks();
    }

    private function getTaskText($taskType): string
    {
    ...
    // Обновить loadTrucks() для плановых задач
    public function loadTrucks(): void
    {
        $this->trucks = Truck::with(['driver', 'currentOrder', 'truckModel', 'plannedTasks'])
            ->orderBy('number')
            ->get();
        $this->freeTrucks = Truck::whereIn('status', ['free', 'completed'])->get();
    }
