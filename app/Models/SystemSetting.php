...
class SystemSetting extends Model
{
    ...
    /**
     * Количество раундов оптимизации маршрутов.
     *
     * Раунд 1 = основные маршруты (лучший отвал для каждого забоя)
     * Раунд 2+ = резервные маршруты (второй по качеству отвал и т.д.)
     *
     * Если основной маршрут недоступен (зона закрыта/переполнена/под обваловкой),
     * WRR автоматически переключится на резервный.
     */
    public static function getRoundsCount(): int
    {
        return (int) static::get('rounds_count', 2);
    }

    public static function setRoundsCount(int $count): void
    {
        // Ограничиваем разумными пределами: 1-5 раундов
        $count = max(1, min(5, $count));
        static::set('rounds_count', $count, 'Количество раундов оптимизации маршрутов (1-5)');
    }

    /**
     * Порог адаптивной балансировки (в процентах заполнения зоны).
     *
     * Если зона отвалa заполнена ≤ порога — разрешаем другим забоям
     * направлять туда самосвалы (минимизация расстояния перевозки).
     * Если > порога — оставляем эксклюзивный режим (один отвал — один забой),
     * чтобы избежать "жадного захвата" и обеспечить равномерное заполнение.
     *
     * По умолчанию 30% — означает, что почти пустая зона может обслуживать
     * несколько забоев одновременно, а заполненная — только один.
     */
    public static function getZoneSharingThreshold(): int
    {
        return (int) static::get('zone_sharing_threshold', 30);
    }

    public static function setZoneSharingThreshold(int $percent): void
    {
        // Ограничиваем 0-100%
        $percent = max(0, min(100, $percent));
        static::set('zone_sharing_threshold', $percent, 'Порог адаптивной балансировки зон (%)');
    }
